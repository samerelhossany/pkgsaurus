...
int create_package(char* name,char* version,char*description,char*content,char* location,char* list_associated)
{
//int fdreg=open("registryFile.txt",O_RDWR);
int fdreg=open(list_associated,O_RDWR);
if(fdreg <0)//check if the registry file is not found
{
printf("\033[31;1mERROR IN \033[3mcreate_package\033[1m: \033[4mregistery file is not found\033[0m");
return -1;
...
memmove(path + strlen(location), name, strlen(name));
path[strlen(location) + strlen(name)] = '\0';
...
printf("\033[35;1m package with name %s has been created successfully in %s directory and %s list\033[0m\n",name,location,list_associated);
printf("\033[35mupdating data int the registry file....\033[0m\n");
char regbuffer[1024];
char temp_buffer[1024];
int pos=0;
...
//sto7y upgradable file
int fdUp = open("upgradeable", O_CREATE|O_RDWR);
if (fdUp < 0) {
    printf("\033[31;1mFailed to create upgradeable file!!!\033[0m\n");

    return -1;
} else {
    printf("\033[34;1mupgradeable file has been created successfully\033[0m\n");
    close(fdUp);
}
...
else
{
    ...
    create_package("package1.pkg","1.0","A dummy package for testing","just a content","server/","registryFile.txt");
    create_package("package2.pkg","1.0","Another dummy package for testing","just another content","server/","registryFile.txt");
    create_package("package3.pkg","2.1","\0","\0","server/","registryFile.txt");
    //executable files
...
create_package("package1.pkg","0.8","A dummy package for testing upgrade","just a content","pkgsaurus_packages/","pkgsaurusList.txt");
